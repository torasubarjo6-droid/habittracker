...
$pageTitle = 'Edit Habit - HabitTrack';
$error     = '';
$habitId   = (int)($_GET['id'] ?? 0);

// Ambil data habit, pastikan milik user ini
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$nama) {
        ...
        $stmt = $pdo->prepare("UPDATE habits SET nama_habit = ?, kategori = ?, warna = ? WHERE id = ? AND user_id = ?");
        $stmt->execute([$nama, $kategori, $warna, $habitId, $_SESSION['user_id']]);
        $_SESSION['flash'] = "Habit '{$nama}' berhasil diperbarui!";
        header('Location: ' . BASE_URL . '/habits/index.php');
        exit;
    }
}

// Isi nilai dari form POST atau dari DB
$nama     = $_POST['nama_habit'] ?? $habit['nama_habit'];
$kategori = $_POST['kategori']   ?? $habit['kategori'];
$warna    = $_POST['warna']      ?? $habit['warna'];
...
<div class="max-w-lg mx-auto">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Habit</h2>
    </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <label class="cursor-pointer">
                        ...
                        <div class="w-8 h-8 rounded-full border-4 border-transparent hover:scale-110 transition"
                             style="background-color: <?= $hex ?>; <?= $checked ? 'border-color:' . $hex : '' ?>"
                             title="<?= $label ?>"
                             onclick="highlightColor('<?= $hex ?>')">
                        </div>
                    </label>
                    <?php endforeach; ?>
                </div>
